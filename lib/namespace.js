...
function NameSpace(manager, nameSpace, options) {
	...
	if (sessionOpt) {
		...
		var sessionStoreObj = {
			host: manager.redis.host,
			port: manager.redis.port
		};
		if (manager.redis.options.pass)
			sessionStoreObj.pass = manager.redis.options.pass;
		var sessionStore = new RedisStore(sessionStoreObj);
	...
	io_namespace.on('connection', function (socket) {
		...
		if (!sessionOpt || auth(session)) {
			...
			socket.on('rely', function (data) {
				if (data.to)
					emit2client(io_namespace, data.to, 'rely', data.message, data.message.room);
				else
