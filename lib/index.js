...
var Server = function(srv, options) {
	...
	if (!options.redis.options || !options.redis.options.pass) options.redis.options = { pass: false };
	var pass = options.redis.options.pass;
	if (pass) {
		pub.auth(pass, function (err) { if (err) throw err; });
		sub.auth(pass, function (err) { if (err) throw err; });
		client.auth(pass, function (err) { if (err) throw err; });
	}
